package com.wx.demo.frameWork.client.wxClient;

import com.wx.demo.bean.CallBack;
import com.wx.demo.tools.CommonUtil;
...
public class WechatSocket {
    private static Logger logger = Logger.getLogger(WechatSocket.class);
    ...
    public WechatSocket(String longServerIp, int port, OnReconnectListener reconnectListener, MsgListener msgListener, CallBack callBack) {
    ...
    public synchronized void sendData(byte[] sendBuff, CallBack back) {
        try {
            while (channel == null) {
                wait(1);
            }
