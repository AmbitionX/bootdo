...
public class WechatUtil {
    ...
    public static String getMac(String usercode) {
        ...
        if (usercode.length() < 12) {
            usercode = WechatUtil.getMd5(usercode.getBytes());
        }
        for (int x = 0; x < 6; x++) {
    ...
    public  static  String getWxDat (String Deviceid,int type){
        String devideId;
        //传进来的是md5
        if(type == 1){
            devideId = Deviceid;
        ...
        //传进来的是62数据
            ...
            devideId = WechatUtil.hexStr2Str(Deviceid.substring(134, 198));
        } else {
            devideId = getMd5(Deviceid);
        }
        String ret = strTo16("49" + devideId.substring(2));
    ...
    public  static  String getDeviceid(String Deviceid,int type){
        String devideId;
        //传进来的是md5
        if(type == 1){
            devideId = Deviceid;
        ...
        //传进来的是62数据
        if(type == 2){
            String devideIdhex = Deviceid.substring(134, 198);
            devideId = WechatUtil.hexStr2Str(devideIdhex);
        }
        //传进来的是其他随机数据
        else {
            devideId = getMd5(Deviceid);
        ...
        //取到最终的md5
        return  "49" + devideId.substring(2);
    }

    public static String getDeviceType(String DevideId,int type) {
        String devideId = DevideId;
        String deviceType;
        if (type == 1) {
            deviceType = getDeviceType(getMac(devideId), null, null);
        } else if (type == 2) {
            deviceType =  getDeviceType(getMac( getDeviceid(DevideId,2)),null,null);
        }else {
            deviceType =  getDeviceType(getMac( getDeviceid(DevideId,0)),null,null);
        }
        return deviceType;
    }
    ...
    public static byte[] getBuffers(WechatMsg msg) {
